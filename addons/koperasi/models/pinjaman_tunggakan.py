...
class PinjamanTunggakan(models.Model):
    _name = 'pinjaman.tunggakan'

    name = fields.Char(string='No Tunggakan', required=True, copy=False, readonly=True, index=True,
    ...
    # master_pinjaman_Tunggakan_id = fields.Many2one('master.pinjaman.Tunggakan.detail', readonly=True)
    # jumlah_kali_jasa = fields.Integer(related='master_pinjaman_Tunggakan_id.jumlah_kali_jasa' ,string='Jumlah Kali Jasa', readonly=True)

    jumlah_tunggakan = fields.Float(string='Jumlah Tunggakan', readonly=True, compute='_compute_tunggakan_')


    sisa_total_pokok = fields.Float(string='Sisa Pokok', compute='_compute_tunggakan_')
    sisa_total_jasa = fields.Float(string='Sisa Jasa', compute='_compute_tunggakan_')
    selisih_total = fields.Float(string='Selisih Total', compute='_compute_tunggakan_')

    tunggakan_detail_ids = fields.One2many('pinjaman.tunggakan.detail', 'tunggakan_id', string='Detail Tunggakan')
    ...
    def action_get_trasaction(self):
        ...
        # moveTunggakanIds = moveObj.search([]).ids
        movetunggakanIds = moveObj.search([('type_pinjaman','=','inbound'),('status','=','unpaid'),('tgl_angsuran','<=',fields.Date.today())])
        # self.Tunggakan_detail_ids = [(4,0,moveTunggakanIds)]buat Many2many
        # tunggakanConfObj = self.env['master.pinjaman.tunggakan'].search([('pinjaman_duration','=',self.jangka_waktu)]).angsuran_detail_ids.filtered(lambda x : x.sisa_angsuran2 <= len(movetunggakanIds) and x.sisa_angsuran1 >= len(movetunggakanIds))
        # self.master_pinjaman_tunggakan_id = tunggakanConfObj.id
        for i , a in enumerate(movetunggakanIds):            
            value = {
                'tunggakan_id' : self.id,
                # 'pinjaman_id' : a.pinjaman_id.id,
                'tgl_angsuran': a.tgl_angsuran,
                'pinjaman_detail_id': a.id,
                'actual_angsuran': a.rencana_angsuran,
                'status': 'unpaid',
        ...
        self.tunggakan_detail_ids = data
        self.state = 'confirmed'
    ...
    def action_validate(self):
        ...
        # for a in self.tunggakan_detail_ids:            
        #     a.pinjaman_detail_id.write({'status' : 'paid','actual_angsuran': a.actual_angsuran,'tunggakan_id': a.tunggakan_id})
    ...
    @api.depends('tunggakan_detail_ids')
    def _compute_tunggakan_(self):
        ...
        for rec in self:
            ...
            # rec.sisa_total_pokok = sum(line.jumlah_pokok for line in rec.Tunggakan_detail_ids.filtered(lambda x: x.status=='paid'))

            

            rec.sisa_total_pokok = sum(line.jumlah_pokok for line in rec.tunggakan_detail_ids)
            rec.sisa_total_jasa = sum(line.jumlah_jasa for line in rec.tunggakan_detail_ids)

            rec.jumlah_tunggakan = 10

            rec.selisih_total = rec.sisa_total_pokok + rec.sisa_total_jasa - rec.jumlah_tunggakan
    ...
    #                 'name': 'Tunggakan Pinjaman',
    #                 'quantity': 1,
    #                 'price_unit': self.jumlah_Tunggakan
    #             })]
    #         }
    ...
    #     else:
    #         raise UserError(_("Jumlah Tunggakan harus lebih besar dari nol..."))

# 
    @api.model
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('pinjaman.tunggakan') or 'New'
        return super(PinjamanTunggakan, self).create(vals)

class PinjamanTunggakanDetail(models.Model):
    _name = 'pinjaman.tunggakan.detail'
    ...
    tunggakan_id = fields.Many2one('pinjaman.tunggakan', required=True)
    
    date = fields.Date(related='tunggakan_id.date', string='Date', readonly=True)
    pinjaman_detail_id = fields.Many2one('pinjaman.details', string='Pinjaman Detail', readonly=True)
    pinjaman_id = fields.Many2one(related='pinjaman_detail_id.pinjaman_id', string='Pinjaman', readonly=True)
    partner_id = fields.Many2one(related='pinjaman_detail_id.partner_id', string='Anggota', readonly=True)
    policy_id = fields.Many2one(related='pinjaman_detail_id.policy_id', string='Master Pinjaman', readonly=True)
    ...
    type_tunggakan = fields.Selection([('cash', 'Cash'), ('transfer','Transfer'), ('payroll','Payroll')],
                             required=True, default='cash')
    jumlah_pokok = fields.Float(related='pinjaman_detail_id.jumlah_pokok_perbulan', string='Jumlah Pokok')
    jumlah_jasa = fields.Float(related='pinjaman_detail_id.jumlah_jasa_perbulan', string='Jumlah Jasa')
    jumlah_angsuran = fields.Float(related='pinjaman_detail_id.jumlah_angsuran', string='Jumlah Angsuran')
